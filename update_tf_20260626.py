"""2026-06-26 daily update: append 2026-06-25 daily row (PV+UV complete)
+ full replace of SRP/inflow/cvr/benchmark/scoreTs/featTs.
Query E (ages) skipped: 2026-06-26 is Friday (Monday-only rule) -> ages untouched.
"""
import json
import sys
from pathlib import Path

TMP = Path("tf-data/tmp_2026_06_26")
PIDS = ["3918642", "3640244", "767440", "2636441", "1930788", "442026"]
REFRESH_FROM = "2026-06-25"
DAILY_CUTOFF = "2026-06-25"  # last date with complete PV+UV data
...
daily = load("daily")
a_rows, b_rows, buv_rows = daily["a"], daily["b"], daily["buv"]
srp = load("srp_cf")
c_rows, f_rows = srp["c"], srp["f"]
d_rows = load("d_a") + load("d_b")
bench_q = load("bench")
h_rows, i_rows = bench_q["h"], bench_q["i"]
g_rows = load("g")
