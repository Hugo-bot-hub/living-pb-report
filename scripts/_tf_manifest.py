...
PIDS_BIG = "3918642, 3640244, 767440, 2636441, 1930788, 442026"
PIDS_STR = "'3918642','3640244','767440','2636441','1930788','442026'"
KW = "'차렵이불','워싱이불','알러지케어','차렵이불단품','이불세트','침구','사계절이불','먼지없는이불'"
YYYYMM = "'202602','202603','202604','202605','202606','202607'"
...
def inflow_q(lo, hi):
  ...
  WHERE date BETWEEN CAST(DATE_ADD('day',{lo},CURRENT_DATE) AS VARCHAR) AND CAST(DATE_ADD('day',{hi},CURRENT_DATE) AS VARCHAR)
    AND object_id IN ({PIDS_STR})
    AND category IN ('IMPRESSION','CLICK') AND object_type='PRODUCTION')
...
GCASE = """CASE WHEN COALESCE(inferred_page_id,page_id) LIKE 'SRP%' THEN '검색'
         WHEN COALESCE(inferred_page_id,page_id)='CATEGORY' THEN '카테고리'
         WHEN COALESCE(inferred_page_id,page_id)='PDP' THEN 'PDP추천'
         WHEN COALESCE(inferred_page_id,page_id) IN ('HOME','SHOPPINGHOME') THEN '홈'
         WHEN COALESCE(inferred_page_id,page_id)='BRAND_DETAIL' THEN '브랜드관'
...
q["A_gmv"] = f"""SELECT CAST(base_dt AS VARCHAR) dt, CAST(product_id AS BIGINT) product_id,
  SUM(gmv) gmv, SUM(gross_profit) gp, SUM(option_quantity) qty
FROM ba_preserved.commerce_gross_profit_orders
WHERE CAST(product_id AS BIGINT) IN ({PIDS_BIG})
  AND yyyymm IN ({YYYYMM}) AND base_dt >= DATE '2026-02-01'
GROUP BY base_dt, product_id ORDER BY base_dt, product_id"""

q["B_funnel"] = f"""SELECT period dt, product_id, SUM(impression_count_all) imp, SUM(pdpview_count_all) pdp, SUM(purchase_count_all) purchase
FROM ba_preserved.comm_product_perform
WHERE product_id IN ({PIDS_BIG}) AND period_base='day' AND period >= '2026-02-01'
  AND yyyymm IN ({YYYYMM})
GROUP BY period, product_id ORDER BY period, product_id"""

q["B_uv"] = f"""SELECT base_dt dt, id pid,
  SUM(impression_user_join) imp_uv, SUM(pdpview_user_join) pdp_uv, SUM(purchase_user_join) buy_uv
FROM ba_preserved.commerce_daily_user_count_v3
WHERE id IN ({PIDS_STR}) AND base = 'product' AND paid_type = 'total'
  AND base_dt >= '2026-02-01'
GROUP BY base_dt, id ORDER BY base_dt, id"""

q["C_srpkw"] = f"""SELECT date, object_id, query_keyword, AVG(object_idx) avg_rank, MIN(object_idx) best_rank, AVG(monthly_sales_score) selling_score
FROM search.commerce_srp_dataset_daily_v0_0_2
WHERE object_id IN ('3918642','3640244') AND date >= CAST(DATE_ADD('day',-14,CURRENT_DATE) AS VARCHAR)
  AND query_keyword IN ({KW})
GROUP BY date, object_id, query_keyword ORDER BY date DESC"""

q["E_ages"] = f"""WITH buyers AS (
  ...
  WHERE o.yyyymm BETWEEN '202501' AND '202607'
    AND CAST(o.product_id AS BIGINT) IN ({PIDS_BIG})
...
WHERE u.birthday IS NOT NULL AND LENGTH(u.birthday) >= 4
  AND CAST(SUBSTR(u.birthday,1,4) AS INT) BETWEEN 1930 AND 2010
GROUP BY 1, 2 ORDER BY 1, 2"""

q["F_matrix"] = f"""SELECT object_id, query_keyword, AVG(object_idx) avg_rank, MIN(object_idx) best_rank,
  AVG(monthly_sales_score) score, SUM(is_clicked)*1.0/COUNT(*) ctr, COUNT(*) imp
FROM search.commerce_srp_dataset_daily_v0_0_2
WHERE object_id IN ({PIDS_STR})
  AND date >= CAST(DATE_ADD('day',-30,CURRENT_DATE) AS VARCHAR)
  AND query_keyword IN ({KW}) AND object_idx <= 100
GROUP BY object_id, query_keyword HAVING COUNT(*) > 10"""

q["D1_inflow"] = inflow_q(-14, -8)
q["D2_inflow"] = inflow_q(-7, -1)

q["G_cvr"] = f"""WITH inflow_click AS (
  SELECT date dt, object_id pid, uuid, user_id, {GCASE} inflow
  FROM log.analyst_log_table
  WHERE date >= CAST(DATE_ADD('day',-7,CURRENT_DATE) AS VARCHAR)
    AND object_id IN ({PIDS_STR}) AND category='CLICK' AND object_type='PRODUCTION'),
product_purchase AS (
  SELECT DISTINCT CAST(base_dt AS VARCHAR) dt, CAST(user_id AS VARCHAR) user_id, CAST(product_id AS BIGINT) pid
  FROM ba_preserved.commerce_gross_profit_orders
  WHERE yyyymm IN ('202607','202606') AND base_dt >= DATE_ADD('day',-7,CURRENT_DATE)
    AND option_quantity > 0 AND user_id IS NOT NULL)
SELECT i.dt, i.pid, i.inflow,
  COUNT(DISTINCT i.uuid) click_uv,
  COUNT(DISTINCT CASE WHEN p.user_id IS NOT NULL THEN i.uuid END) buy_uv
FROM inflow_click i LEFT JOIN product_purchase p
  ON i.dt=p.dt AND CAST(i.user_id AS VARCHAR)=p.user_id AND CAST(i.pid AS BIGINT)=p.pid
GROUP BY i.dt, i.pid, i.inflow"""

q["scoreTs"] = f"""SELECT date dt, object_id pid, AVG(monthly_sales_score) score
FROM search.commerce_srp_dataset_daily_v0_0_2
...
q["srpFeatureTs"] = f"""SELECT date dt, object_id pid, AVG(review_count_score) review, AVG(sell_cnt_28_day_score) sell28,
...
q["H_bands"] = """SELECT
  CASE WHEN p.brand_name='오늘의집 layer' THEN 'PB' ELSE '3P' END pb_flag,
  CASE WHEN p.selling_cost < 30000 THEN '01_<30k'
       WHEN p.selling_cost < 50000 THEN '02_30-50k'
       WHEN p.selling_cost < 70000 THEN '03_50-70k'
       WHEN p.selling_cost < 100000 THEN '04_70-100k'
       ELSE '05_100k+' END price_band,
  COUNT(DISTINCT p.product_id) prod_cnt,
...
q["I_lineup"] = """SELECT id pid, SUM(impression_user_join) imp_uv, SUM(pdpview_user_join) pdp_uv, SUM(purchase_user_join) buy_uv
FROM ba_preserved.commerce_daily_user_count_v3
WHERE base_dt BETWEEN CAST(DATE_ADD('day',-7,CURRENT_DATE) AS VARCHAR) AND CAST(DATE_ADD('day',-1,CURRENT_DATE) AS VARCHAR)
  AND base = 'product' AND paid_type = 'total'
  AND id IN ('3918642','4037745','3640244','3640123','3707436','3508402','3707457')
GROUP BY 1"""

with open(r"C:\Users\yys\living-pb-report\scripts\_tf_jobs.json", "w", encoding="utf-8") as f:
    json.dump(q, f, ensure_ascii=False, indent=1)
print("manifest written:", len(q), "queries")
