"""2026-06-18 daily update: partial daily refresh (2026-06-10~06-16, UV lags to 06-16)
+ full replace of SRP/inflow/cvr/benchmark/scoreTs/featTs.
Query E (ages) skipped: 2026-06-18 is Thursday (Monday-only rule).
"""
import json
import sys
from pathlib import Path

TMP = Path("tf-data/tmp_2026_06_18")
PIDS = ["3918642", "3640244", "767440", "2636441", "1930788", "442026"]
REFRESH_FROM = "2026-06-10"
DAILY_CUTOFF = "2026-06-16"  # last date with complete PV+UV data (UV lags PV by 1d)


def load(name):
    with open(TMP / f"query_{name}.json", encoding="utf-8") as f:
        return json.load(f)
...
score_rows, feat_rows = load("score"), load("feat")
...
# Append only when funnel PV + UV both exist (all-fields rule).
appended = {}
for pid in PIDS:
