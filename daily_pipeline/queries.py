...
QUERIES['daily_gmv'] = f"""
SELECT CAST(base_dt AS VARCHAR) dt, CAST(product_id AS BIGINT) pid, SUM(gmv) gmv,
  -- SUM(double) 은 병렬 합산 순서에 따라 ±1원 흔들림 → DECIMAL 합산으로 결정적·정확하게
  CAST(SUM(CAST(gross_profit AS DECIMAL(38,6))) AS DOUBLE) gp, SUM(option_quantity) qty
...
# ============ SRP: base (노출가중 실측 평균순위·진짜P5·노출·CTR·주간판매) — matrix+keywordRank 공통 소스 ============
# 🔴 2026-07-21 소스 교체: 구 commerce_srp_dataset_daily_v0_0_2 07-06 동결.
#   🎯 정본 = search.query_object_metrics_with_order_mart (SRP 마트, 07-19 신선). 구 v0_0_2와 동일 계보:
#     avg_object_idx = 노출가중 실제 평균순위(클릭편향 없음), n_impression/n_clicked/n_purchased 보유.
#   🎯 P5 최고순위 = search.search_object_with_order_mart (노출단위 원본, object_idx per 노출) approx_percentile(0.05).
#   ✅ 검증: 마트 노출가중 순위가 동결 전 v0_0_2 실제값과 일치(studio매트 21.95→20.39). att_log 클릭방식(16.3)의 낙관편향 제거.
#   오가닉만 = source='STORE_SRP' AND object_section='스토어 검색 결과'(광고·유사상품·중간광고 제외). 최근 7일. rank/best 0-based.
#   상품별 top-15(노출순) = 1000행캡 내 유지. rank=노출가중avg, best=P5.
QUERIES['srp_base14'] = f"""
WITH agg AS (
  SELECT query_keyword kw, object_id pid,
    SUM(avg_object_idx*n_impression)/SUM(n_impression) rank,
    SUM(n_impression) imp, SUM(n_clicked) clk, SUM(n_purchased) buy
  FROM search.query_object_metrics_with_order_mart
...
p5 AS (
  ...
  GROUP BY query_keyword, object_id),
j AS (
  SELECT a.pid, a.kw, a.rank, COALESCE(p.best, CAST(round(a.rank) AS INT)) best, a.buy, a.imp, a.clk,
    ROW_NUMBER() OVER (PARTITION BY a.pid ORDER BY a.imp DESC, a.kw) rn
  FROM agg a LEFT JOIN p5 p ON a.kw=p.kw AND a.pid=p.pid)
SELECT pid, kw, ROUND(rank,2) rank, CAST(best AS INT) best, buy score,
  ROUND(clk*1.0/NULLIF(imp,0),4) ctr, imp
FROM j WHERE rn <= 15 ORDER BY pid, imp DESC, kw"""
...
pdp AS (
  ...
  GROUP BY 1,2,3),
leads AS (
  SELECT COALESCE(s.uid,p.uid) uid, COALESCE(s.pid,p.pid) pid, COALESCE(s.cm,p.cm) cm,
    COALESCE(s.is_cart,0) cart, COALESCE(s.is_scrap,0) scrap, CASE WHEN COALESCE(p.vd,0)>=2 THEN 1 ELSE 0 END revisit
  FROM scr s FULL OUTER JOIN pdp p ON s.uid=p.uid AND s.pid=p.pid AND s.cm=p.cm),
ord AS (
...
SELECT date_format(l.cm,'%Y-%m') ym,
  ...
  COUNT(*) leads, SUM(cv.bought) conv, SUM(cv.gmv) gmv
...
# ⚠️ 상위 N 컷오프는 ORDER BY 에 완전순서(users→gmv→pid)를 줘야 함. users 만으로 자르면
#    동점 상품이 런마다 교대로 표에 들락거림(2026-07-16 실측).
...
agg AS (SELECT tier, pid, brand_name, COUNT(DISTINCT uid) users, SUM(qty) qty, SUM(gmv) gmv,
  ROW_NUMBER() OVER (PARTITION BY tier ORDER BY COUNT(DISTINCT uid) DESC, SUM(gmv) DESC, pid) rn FROM j GROUP BY tier, pid, brand_name)
...
agg AS (SELECT tier, pid, product_name, brand_name, COUNT(DISTINCT uid) users, SUM(gmv) gmv,
  ROW_NUMBER() OVER (PARTITION BY tier ORDER BY COUNT(DISTINCT uid) DESC, SUM(gmv) DESC, pid) rn FROM j GROUP BY tier, pid, product_name, brand_name)
...
agg AS (SELECT pid, product_name, brand_name, COUNT(DISTINCT uid) users, SUM(gmv) gmv, ROW_NUMBER() OVER (ORDER BY COUNT(DISTINCT uid) DESC, SUM(gmv) DESC, pid) rn FROM j GROUP BY 1,2,3)
SELECT (SELECT COUNT(*) FROM fu) buyers, pid, product_name, brand_name, users, gmv FROM agg WHERE rn<=8 ORDER BY users DESC, gmv DESC, pid"""
...
agg AS (SELECT pid, product_name, brand_name, COUNT(DISTINCT uid) users, SUM(gmv) gmv, ROW_NUMBER() OVER (ORDER BY COUNT(DISTINCT uid) DESC, SUM(gmv) DESC, pid) rn FROM j GROUP BY 1,2,3)
SELECT (SELECT COUNT(*) FROM mu) buyers, pid, product_name, brand_name, users, gmv FROM agg WHERE rn<=8 ORDER BY users DESC, gmv DESC, pid"""
