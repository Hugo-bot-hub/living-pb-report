...
& $py -c "import daily_pipeline.queries as q,json,datetime; d=dict(q.QUERIES); [d.pop(k,None) for k in (('ages','lead_growth','lead_cohort_ts','lead_attribution') if datetime.date.today().weekday()!=0 else ())]; json.dump(d,open('daily_pipeline/_queries.json','w',encoding='utf-8'),ensure_ascii=False); print('dumped',len(d))" *>> $log
